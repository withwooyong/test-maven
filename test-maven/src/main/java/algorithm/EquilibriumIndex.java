...
public class EquilibriumIndex {
	...
	public static void main(String[] args) {
		...
		//System.out.println(equilibrium1(arr, arr_size));
	 ...
	 */
	private static int equilibrium1(int arr[], int n) { //  O(n^2)
		int i, j;
	 ...
	 * a) Update sum to get the right sum. sum = sum - arr[i] // sum is now right sum 
	 * b) If leftsum is equal to sum, then return current index. 
	 * c) leftsum = leftsum + arr[i] // update leftsum for next iteration. 
	 * 4) return -1 // If we come out of loop without returning then // there is no equilibrium index CJava
	 */
	private static int equilibrium2(int arr[], int n) {
