...
%>
<%
System.out.println("[" + IFuncDate.getSystemTime() + "] SoapToWas 시작 >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
...
Connection gConn = null;
	...
	File file = null;
	File manualFile = null; //20110221 cjyu ManualRequest처리를 위한 파일객체
...
System.out.println("[" + IFuncDate.getSystemTime() + "] SoapToWas CCS_IF001 1-0 처리 호출 [EPG전송] >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
System.out.println("index_str >>>" + index_str);
			if(index_str.substring(0, 1).equals("S")) { // 편성정보
System.out.println("[" + IFuncDate.getSystemTime() + "] SoapToWas CCS_IF001 1-1 편성정보 호출 [EPG전송] >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
				...
				*/
				...
				{
System.out.println("[" + IFuncDate.getSystemTime() + "] SoapToWas 요청편성일자["+arr_str[2]+"] 기존파일로 전달 [EPG전송] >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
...
//				    	gConn = IDBConnection.getConnection();
	...
	System.out.println("[" + IFuncDate.getSystemTime() + "] SoapToWas CCS_IF001 1-2 편성정보 처리 [EPG전송] >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
					} catch (Exception e) {
						try {
						  e.printStackTrace();
						  gConn.rollback();
...
System.out.println("[" + IFuncDate.getSystemTime() + "] SoapToWas CCS_IF001 1-1 채널정보 호출 [EPG전송] >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
				biz_ch = new BIZ_CH();
...
System.out.println("[" + IFuncDate.getSystemTime() + "] SoapToWas CCS_IF001 1-2 채널정보 처리 [EPG전송] >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
			}
		} else if(ID_CCS_IF.equals("CCS_IF002")) { // DBS처리
System.out.println("[" + IFuncDate.getSystemTime() + "] SoapToWas CCS_IF002 1-0 처리 호출 [DBS처리] >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
			aFilePath = "";
...
//		    	gConn = IDBConnection.getConnection();
		    	gConn = IDBConnection.getConnection(BIZ_COMMON.CCS_DS_NAME);
System.out.println("[" + IFuncDate.getSystemTime() + "] SoapToWas CCS_IF002 1-1 처리 성공 [DBS처리] >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
...
System.out.println("[" + IFuncDate.getSystemTime() + "] SoapToWas CCS_IF002 1-2 처리 실패 [DBS처리] >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
				try {
				  e.printStackTrace();
				  gConn.rollback();
				}
...
System.out.println("[" + IFuncDate.getSystemTime() + "] SoapToWas CCS_IF002 1-2 처리 성공 [DBS처리] >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
				IDBConnection.setAutoCommit(gConn, true);
				IDBConnection.close(gConn);
			}
...
//				gConn = IDBConnection.getConnection();
			    	gConn = IDBConnection.getConnection(BIZ_COMMON.CCS_DS_NAME);
System.out.println("[" + IFuncDate.getSystemTime() + "] SoapToWas CCS_IF009 1-1 DB 연결 성공 [고객인증요청] >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
				aFilePath = "";
				if ( request.getParameter("aFilePath") != null ) {//DBS 수신파일 저장 위치
					aFilePath = IFuncStr.no(request.getParameter("aFilePath").trim());
				}
...
System.out.println("[" + IFuncDate.getSystemTime() + "] SoapToWas CCS_IF009 1-2 처리 실패 [고객인증요청] >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
				try {
				  e.printStackTrace();
				}
				catch (Exception e1) {}
...
System.out.println("[" + IFuncDate.getSystemTime() + "] SoapToWas CCS_IF009 1-2 처리 종료 [고객인증요청] >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
			}
		}
	}
	catch (Exception e) {
...
System.out.println("[" + IFuncDate.getSystemTime() + "] SoapToWas 정상 종료>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
	}
%>
