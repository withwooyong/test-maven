...
/**
 * Peaks Divide an array into the maximum number of same-sized blocks, each of
 ...
 * > A[P + 1].
 * 
 * For example, the following array A: A[0] = 1 A[1] = 2 A[2] = 3 A[3] = 4 A[4]
 * = 3 A[5] = 4 A[6] = 1 A[7] = 2 A[8] = 3 A[9] = 4 A[10] = 6 A[11] = 2 has
 ...
 * be divided. Array A can be divided into blocks as follows: one block (1, 2,
 * 3, 4, 3, 4, 1, 2, 3, 4, 6, 2). This block contains three peaks. two blocks
 * (1, 2, 3, 4, 3, 4) and (1, 2, 3, 4, 6, 2). Every block has a peak. three
 * blocks (1, 2, 3, 4), (3, 4, 1, 2), (3, 4, 6, 2). Every block has a peak.
 * 
 ...
 * A[5].
 * 
 * The maximum number of blocks that array A can be divided into is three. Write
 * a function: class Solution { public int solution(int[] A); } that, given a
 * non-empty zero-indexed array A consisting of N integers, returns the maximum
 * number of blocks into which A can be divided.
 * 
 * If A cannot be divided into some number of blocks, the function should return
 * 0. For example, given: A[0] = 1 A[1] = 2 A[2] = 3 A[3] = 4 A[4] = 3 A[5] = 4
 * A[6] = 1 A[7] = 2 A[8] = 3 A[9] = 4 A[10] = 6 A[11] = 2 the function should
 * return 3, as explained above.
 * 
 * Assume that: N is an integer within the range [1..100,000]; each element of
 * array A is an integer within the range [0..1,000,000,000].
 * 
 * Complexity: expected worst-case time complexity is O(N*log(log(N))); expected
...
public class Peaks {
	...
	public static int solution(int[] A) {
		...
		log.debug("peaks={} peakCount={}", peaks.toString(), peakCount);
	...
	public static int mySolution(int[] A) {
		...
		for (int i = 1; i <= N; i++) {
			...
			if (N % i == 0 || blocks <= peaks.size()) { // 나누어서 0으로 떨어져야 same
														// number of elements
				boolean ok = true;
				int threshold = 0;
				for (int j = 0; j < peaks.size(); j++) {
