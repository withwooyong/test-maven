/*
	...
	N 개의 정수로 구성된 두 개의 비어 있지 않은 제로 색인 배열 A와 B가 제공됩니다. 이 배열은 N 널빤지를 나타냅니다. 보다 정확하게 A [K]는 K 번째 판자의 시작이고 B [K]는 K 번째 판자의 끝입니다.
다음에는 M 개의 정수로 구성된 비어 있지 않은 0 인덱스 배열 C가 주어집니다. 이 배열은 M 개의 못을 나타냅니다. 보다 정확하게, C [I]는 I-th 못에서 망치질 수있는 위치입니다.
A [K] ≤ C [I] ≤ B [K]가되도록 손톱 C [I]가 존재한다면 판자 (A [K], B [K])가 못 박히게된다.
목표는 모든 판자가 못 박힐 때까지 사용해야하는 최소 개수의 못을 찾는 것입니다. 다른 말로하면 첫 번째 J 손톱 만 사용한 후에 모든 널빤지가 못을 박을 수 있도록 J 값을 찾아야합니다. 보다 정확하게, 0 ≤ K <N이되는 모든 널빤지 (A [K], B [K])에 대해 I <J이고 A [K] ≤ C [I] ≤ 인 못 C [I] B [K].
