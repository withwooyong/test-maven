/*
	...
	N 개의 정수 배열을 감안할 때, I 번째 디스크는 (0, I)에 중심을두고 A [I]의 반경을 갖도록 2D 평면에 N 개의 디스크를 그립니다. J 번째 디스크와 K 번째 디스크는 J ≠ K이고 J 번째와 K 번째 디스크는 적어도 하나의 공통점을 가지고 있다고 가정합니다.
함수 작성 :
int 솔루션 (int A [], int N);
위에서 설명한대로 N 개의 디스크를 설명하는 배열 A가 주어진다면 교차하는 디스크 쌍의 수를 반환합니다. 예를 들어, 주어진 N = 6 및 :
A [0] = 1A [1] = 5A [2] = 2
A [3] = 1A [4] = 4A [5] = 0
