...
아래 그림과 같이 상대적 높이가 배열 A로 표시되는 산 범위로 여행 할 것입니다. 당신은 당신과 함께 가져 가야 할 깃발의 개수를 선택해야합니다. 목표는 특정 규칙에 따라 피크에서 최대 플래그 수를 설정하는 것입니다.

플래그는 최고점에서만 설정할 수 있습니다. 또한 K 플래그를 사용하면 두 플래그 사이의 거리가 K보다 크거나 같아야합니다. 인덱스 P와 Q 사이의 거리는 절대 값 | P - Q |입니다.
