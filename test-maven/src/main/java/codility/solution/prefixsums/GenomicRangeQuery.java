...
DNA 시퀀스는 N 개 문자로 구성된 비어 있지 않은 문자열 S = S [0] S [1] ... S [N-1]로 주어집니다. 비어 있지 않은 배열 P와 Q에 각각 M 개의 정수로 구성된 M 개의 쿼리가 있습니다. K 번째 질의 (0 ≤ K <M)는 위치 P [K]와 Q [K] (포함) 사이의 DNA 서열에 포함 된 뉴클레오타이드의 최소 영향 인자를 찾도록 요구합니다.
