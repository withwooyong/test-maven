...
/*
 * http://telsome.tistory.com/entry/CountMultiplicativePairs
 * 
...
public class CountMultiplicativePairs {
	...
	private static Logger log = LoggerFactory.getLogger(CountMultiplicativePairs.class);

	public static void main(String[] args) {
		int[] A = { 0, 1, 2, 2, 3, 5 };
		int[] B = { 500000, 500000, 0, 0, 0, 20000 };
		
		log.debug("{}", solution(A, B)); // return 8
	}
	...
	public static int solution(int[] A, int[] B) {
		...
		for (int P = 0; P < N; P++) {
			for (int Q = P + 1; Q < N; Q++) {
				// A pair of indices (P, Q) is multiplicative if 0 ≤ P < Q < N 
				// and C[P] * C[Q] ≥ C[P] + C[Q].
				if (C[P].multiply(C[Q]).compareTo(C[P].add(C[Q])) >= 0) {
					count++;
