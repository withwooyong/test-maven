...
/*
 * http://telsome.tistory.com/entry/BugfixingLeaderSorted
 *  * 
 ...
 * 
 * Unfortunately, despite the fact that the function may return expected result for the example input, there is a bug in the implementation, which may produce incorrect results for other inputs. 
...
public class BugfixingLeaderSorted {
	...
	private static Logger log = LoggerFactory.getLogger(BugfixingLeaderSorted.class);
	
	public static void main(String[] args) {
		//int[] A = { 2, 2, 2, 2, 2, 3, 4, 4, 4, 6 }; // return −1
		int[] A = { 1, 1, 1, 1, 50 }; // return 1
	...
	private static int solution(int[] A) {
		...
		if (n % 2 == 0 ? count > pos : count > pos - 1) { // 여기수정한듯..
			return candidate;
		}
		return (-1);
	}
