/*
	...
	작은 개구리가 강 저편에 도착하기를 원합니다. 개구리는 처음에 강 하나 은행 (위치 -1)에 위치하고 다른 은행 (위치 N)에 도착하려고합니다. 
	개구리는 어떤 거리 F (K)로 점프 할 수 있습니다. 
	여기서 F (K)는 K 번째 피보나치 수입니다. 
	운 좋게도 강가에 많은 잎이 있고, 개구리는 잎 사이를 뛰어 넘을 수 있지만 위치 N의 은행 방향으로 만 갈 수 있습니다.
	강에있는 잎은 N 개의 정수로 구성된 0 인덱스 배열 A로 표현됩니다. 
	배열 A의 연속 요소는 강에서 0에서 N-1까지의 연속적인 위치를 나타냅니다. 
	배열 A는 0 및 / 또는 1 만 포함합니다.
	0은 잎이없는 위치를 나타내고;
	1은 잎을 포함하는 위치를 나타낸다.
	목표는 개구리가 강 저편 (위치 -1에서 위치 N까지)으로 갈 수있는 점프의 최소 횟수를 세는 것입니다. 
	개구리는 -1과 N 사이 (강 유역)와 잎이있는 모든 위치 사이를 이동할 수 있습니다.
	...
	즉, N 개의 정수로 구성된 인덱스가없는 배열 A가 주어지면, 개구리가 강 저편에 도달 할 수있는 최소 점프 수를 반환합니다. 
	개구리가 강 저편에 도달 할 수 없다면 함수는 -1을 반환해야합니다.
