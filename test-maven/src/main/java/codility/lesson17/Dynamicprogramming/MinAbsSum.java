 ...
 * computes the minimum value of val(A,S) from all possible values of val(A,S) for 
 ...
 * since for S = [−1, 1, −1, 1], val(A, S) = 0, which is the minimum possible value.
 * 
 * Assume that:
...
public class MinAbsSum {
	...
	public static int solution(int[] a) {
		...
		for (int i = 1; i < counts.length; i++) {
			// we check r[j]. if it's not less than 0, then it means we've
			// reached j value with previous steps, so no need to spend current
			// if it's less than 0, spend 1 current number if r[j - i] has been
			// reached
			for (int j = 0; j < r.length; j++) {
				// negative value means we haven't reached this value, so we
				// have to spend 1 current if we can
				if (r[j] >= 0) {
					r[j] = counts[i];
				} else if (j - i >= 0 && r[j - i] > 0) {
					r[j] = r[j - i] - 1;
				}
				// the value in r[j] then means how many of the current values
				// are left when we reached the value j
			}
		}
		int result = sum;
		// don't have to traverse all the arrays, since i - the sum of elements.
		// if it's reachable then (sum - i) - reachable as well.
		// so if the value is reachable then the diff is abs(i - (sum - i)),
		// which is the same as abs(sum - 2 * i)
