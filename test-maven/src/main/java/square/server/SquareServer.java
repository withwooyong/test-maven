...
import square.SquareResult;
import square.Squarer;

/**
 * SquareServer is a server that squares integers passed to it. It accepts
 * requests of the form: Request ::= Number "\n" Number ::= [0-9]+ and for each
 * request, returns a reply of the form: Reply ::= (Number | "err") "\n" where a
 * Number is the square of the request number, or "err" is used to indicate a
 * misformatted request. SquareServer can handle only one client at a time.
 */
...
//	        // read the reply
//	        System.out.println("replies.take()=" + replies.take());
