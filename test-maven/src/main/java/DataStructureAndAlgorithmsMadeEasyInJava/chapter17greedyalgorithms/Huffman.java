...
public class Huffman {
	...
	private int SIZE;
	private int[] labels = null; // Label associated with each node, maybe ASCII
									// characters
	private int[] freq = null; // Frequency array
	private String[] huffCodes = null; // Huffman string for each label
	private boolean[][] huffBoolCodes = null; // Huffman boolean codes for each
												// label
	...
	public boolean[][] getHuffmanBoolCodes() {
		return huffBoolCodes;
	}
	...
	private String[] buildHuffmanTree() {
		...
		for (int i = 0; i < SIZE; i++) {
			if (freq[i] > 0) { // If the character doesnt appear in the input
								// text
	...
	private void setHuffmanCodes(HuffmanTreeNode node, String code) {
		if ((node.getLeftNode() == null) && (node.getRightNode() == null)) {
			...
			huffCodes[node.getCharacter()] = code; // Storing the huff codes in
													// an array
