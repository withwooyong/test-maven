	...
	public static ListNode reverseKNodes(ListNode head, int k) {
		...
		while (current != null) {
			...
			if (prevTail != null) {
				...
				// We just reversed first set of K nodes, update head point to
				// the Kth Node
				head = tail;
			}
			// save the last node after reverse since we need to connect to the
			// next set.
			prevTail = prevCurrent;
			// Save the current node, which will become the last node after
			// reverse
			prevCurrent = current;
		}
		return head;
